...
function SixthChart(props) {
    const [facultyName, setFacultyName] = useState('Ren,Bin');
    ...
    const fetchKeywords = async () => {
        try {
            const response = await axios.get('http://127.0.0.1:5000/faculty/keywords', {
                params: { name: facultyName },
            });
    ...
    const handleDelete = async (row) => {
        try {
            await axios.post('http://127.0.0.1:5000/faculty/keywords/delete', {
                name: facultyName,
                keyword: row,
            });
            fetchKeywords();
    ...
    const handleAddKeyword = async () => {
        try {
            await axios.post('http://127.0.0.1:5000/faculty/keywords/add', {
                name: facultyName,
    ...
    return (
        <DashboardBox>
            <BoxHeader
            title="Manage Keywords of a faculty Member"
